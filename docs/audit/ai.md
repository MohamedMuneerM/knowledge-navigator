...
**Before:** 89 chapters and 1,057 topics, with no levels, priorities, summaries or prerequisites. Deep learning and computer vision were well covered. Several standard areas were missing or had only a single line: logic and inference, constraint satisfaction, automated planning, multi-agent systems, speech, recommender systems, time series, causal inference, graph neural networks, LLM application engineering, most robot platforms, and a beginner on-ramp. Coverage was about 70% of a full AI/ML/robotics curriculum.

**After:** 120 chapters (31 new) and 1,656 topics in 22 categories. Every chapter now has a level, priority, one-sentence summary and minimal prerequisites. The validator reports 0 errors and 0 warnings. Against AIMA 4e, the ACM CCS AI/ML subtree, the Stanford core courses and the main robotics texts, coverage is now essentially complete. The remaining gaps are niche and are listed in section 7.
...
- `ai-deep-learning-frameworks` moved from Systems to Deep Learning, since learners use PyTorch while they learn DL.
...
Some chapters were renamed for clarity without changing their meaning:
...
Topic typos were fixed: "A\\" → "A*", "RRT\\" → "RRT*", "D and D Lite" → "D* and D* Lite", "Informed search (A, …)" → "(A*, …)".

Two long flat lists were restructured, with every id kept:
...
About 200 topics were also added to thin existing chapters, including:
...
- The main spine runs Introduction to AI & ML → Programming & Tools + Maths & Stats for DS → Core ML Concepts → Types of ML → Neural Network Foundations → Regularization → CNNs / RNNs → Transformers → LLMs → Fine-tuning, VLMs, Frontier.
- The classical AI branch runs Core AI Concepts → Classical AI (search) → Logic & Automated Reasoning → KR&R / Automated Planning, with a side branch Classical AI → Constraint Satisfaction.
- The uncertainty and RL branch runs Probabilistic AI (needs `ma-probability-theory`) → Probabilistic ML → Probabilistic Programming, and Core ML → RL Fundamentals → DP → Monte Carlo → Model-Free → Deep RL → Advanced RL.
- The LLM engineering fast track needs no heavy maths: Introduction → Prompt Engineering (L2) → LLM Application Engineering → RAG → Agents → LLM Evaluation. The full route into LLM internals goes through Transformers.
- The robotics spine runs Robot Fundamentals (L1) → Actuators & Sensors / ROS → Kinematics & Dynamics (needs `ma-linear-algebra`, `ph-classical-mechanics`) → Control Systems → Motion Planning → Perception → Mobile / Aerial / Legged → Humanoids. The Iron Man path is Control Systems + HRI → Wearable Robotics & Exoskeletons, and Speech → Conversational AI → Wearable AI Assistants & AR.
- The deepest chain in the discipline has 11 stages.

## 7. Recommended moves / open questions

- These chapters would fit a future CS discipline:
  ...
  They stay here for now. Once CS exists, they should be linked with `related`, and the CS programming and algorithms chapters should become prerequisites of `ai-programming-tools`, the Classical AI chapter and `ai-robot-software-ros`.
...
- These overlaps are kept and linked with `related`:
  - Classical AI ↔ Constraint Satisfaction / Automated Planning (the old chapter keeps its overview topics, and the new chapters go deep)
...
- Overlaps with other disciplines are also linked via `related`. The robotics chapters link to the electronics robotics chapters (`el-robot-operating-system-ros`, `el-kinematics-dynamics`, `el-robot-control`, `el-simultaneous-localization-and-mapping-slam`, `el-autonomous-driving`). Control links to `ma-control-theory` / `el-control-theory`, the theory chapters to `ma-information-theory`, `ma-statistical-learning-theory` and `ma-optimization-algorithms`, and edge AI to `el-tinyml-tiny-machine-learning`.
- Medical/surgical, agricultural, space, underwater and micro/nano robotics are still thin. They get survey-level coverage only, all inside `ai-types-of-robots-applications`. Dedicated chapters (e.g. "Medical & Surgical Robotics") could be added later.
