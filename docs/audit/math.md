...
- **Cross-discipline prerequisites for phase B** (the targets don't exist in the legacy files yet):
  - ma-error-analysis, ma-monte-carlo-methods, ma-computer-algebra, ma-algorithm-analysis -> "programming fundamentals (cs)"
  - ma-formal-verification -> "programming languages / semantics (cs)"
  - ma-mathematical-biology -> "introductory biology (biology)"
  - ma-mathematical-chemistry -> "general chemistry (chemistry)"
  - ma-solid-mechanics -> "mechanics of materials (mechanical)"
  - ma-classical-mechanics -> "introductory mechanics (physics)"
