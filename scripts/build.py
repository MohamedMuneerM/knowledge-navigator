...
def load_discipline(did, errors, write):
    ...
    for path in sorted(ddir.glob('*.json')):
        ...
        if dumps(ch) != path.read_text(encoding='utf-8'):
            unformatted += 1
            if write:
                path.write_text(dumps(ch), encoding='utf-8')
        chapters.append(ch)
...
def main():
    ...
    for did in manifest['disciplines']:
        meta, n = load_discipline(did, errors, write=not check_only)
    ...
    (ROOT / 'knowledge_base.json').write_text(json.dumps(data, ensure_ascii=False, separators=(',', ':')), encoding='utf-8')
